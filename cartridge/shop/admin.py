...
product_fieldsets[0][1]["fields"].extend(["position", "vendor", "second_hand", "categories", "content",("length","width","height"),"weight"])
product_fieldsets = list(product_fieldsets)
product_fieldsets.append((_("Other products"), {
    "classes": ("collapse-closed",),
    "fields": ("related_products", "upsell_products")}))

product_list_display = ["admin_thumb", "title", "status", "position", "available",
                        "admin_link"]
product_list_editable = ["status", "position", "available"]
